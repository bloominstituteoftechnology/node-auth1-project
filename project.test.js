...
describe('server.js', () => {
  describe('[POST] /api/auth/login', () => {
    ...
    it('a "chocolatechip" cookie gets set on the client on valid credentials', async () => {
      const res = await request(server).post('/api/auth/login').send({ username: 'bob', password: '1234' })
      const cookies = setCookie.parse(res, { map: true })
      expect(cookies.chocolatechip).toMatchObject({ name: 'chocolatechip' })
    }, 500)
    it('no cookie gets set on invalid credentials (saveUninitialized=false)', async () => {
  ...
  describe('[POST] /api/auth/register', () => {
    ...
    it('new user passwords are saved correctly bcrypted', async () => {
      await request(server).post('/api/auth/register').send({ username: 'sue', password: '1234' })
      const sue = await db('users').where('username', 'sue').first()
      expect(bcrypt.compareSync('1234', sue.password)).toBeTruthy()
    }, 500)
    it('no cookie gets set by registering (saveUninitialized=false)', async () => {
      const res = await request(server).post('/api/auth/register').send({ username: 'sue', password: '1234' })
      const cookies = setCookie.parse(res, { map: true })
      expect(cookies).toEqual({}) // no SET-COOKIE
    }, 500)
    it('responds with the user (user_id and username)', async () => {
      const res = await request(server).post('/api/auth/register').send({ username: 'sue', password: '1234' })
      expect(res.body).toMatchObject({ user_id: 2, username: 'sue' })
    }, 500)
    it('responds with the proper status code and message on "username taken"', async () => {
    ...
    it('responds with the proper status code and message on too short a password', async () => {
  ...
  describe('[GET] /api/auth/logout', () => {
    it('if there is a session it is destroyed so "chocolatechip" cookie not effective anymore', async () => {
    ...
    it('responds with the proper message if the user was not actually logged in', async () => {
  ...
  describe('[GET] /api/users', () => {
    ...
    it('responds with the users if there is a session matching the "chocolatechip" cookie', async () => {
      let res = await request(server).post('/api/auth/login').send({ username: 'bob', password: '1234' })
      const { chocolatechip } = setCookie.parse(res, { map: true })
      res = await request(server).get('/api/users')
        .set('Cookie', `${chocolatechip.name}=${chocolatechip.value}`)
